...
The function cut_number divides into the quantiles automatically.

```{r}
weather_class <- weather %>%
  mutate(strength_gtex_class = cut_number(
    strength_gtex,
    n = 3,
...
```
```{r}
quantile(weather$strength_gtex, 0.33)
quantile(weather$strength_gtex, 0.66)
```
...
denominator <- 45+20+6+20+32+18+14+26+30
...
All three lines are overlapping so we only see the blue line. 
